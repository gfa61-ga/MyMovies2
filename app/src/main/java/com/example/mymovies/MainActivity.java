...
public class MainActivity extends AppCompatActivity implements MovieAdapter.OnClickHandler,
    ...
    // Indicates to load favorite movies from the local database
    private final String SORT_BY_FAVORITES = "favorites";

    private ArrayList<String>[] mApiResponsesJson ; // moviePages API responses saved in MainViewModel
    private MainViewModel mMainActivityModel;
    private LoaderManager mLoaderManager;

    private AppDatabase favoriteMoviesDatabase;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Initialize favoriteMoviesDatabase member variable
        ...
        if (savedInstanceState != null) { // Restores sortByApiPath after orientation change
        ...
        mApiResponsesJson = mMainActivityModel.getmApiResponceJson();
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        mMainActivityModel.setMoviesOfModel(moviesDisplayAdapter.getMovies());
    ...
    public Loader<String> onCreateLoader(int id, @Nullable Bundle args) {
        return new AsyncTaskLoader<String>(this){
            ...
            protected void onStartLoading() {
                ...
                if(mApiResponsesJson[index].size()<Integer.parseInt(apiPageToQuery) ){
                    showMoviesDisplayRecyclerView();
                    ...
                    deliverResult(mApiResponsesJson[index].get(Integer.parseInt(apiPageToQuery)-1));
                }
            }
            ...
            public void deliverResult(String apiResponse) {
                if (apiResponse != null) { // If apiResponse is null, DON'T save it in mApiResponceJson
                    ...
                    if (mApiResponsesJson[index].size() < Integer.parseInt(apiPageToQuery)) {
                        mApiResponsesJson[index].add(Integer.parseInt(apiPageToQuery) - 1, apiResponse);
                    } else {
                        mApiResponsesJson[index].set(Integer.parseInt(apiPageToQuery) - 1, apiResponse);
                    }
                }
    ...
    private void loadNextPageOfMovies() {
        if (!sortByApiPath.equals(SORT_BY_FAVORITES)) { /* Create or restart
                the anonymous AsyncTaskLoader instance to load the movies */
        ...
        } else {  /* Load the movies from the localL favoriteMoviesDatabase */
            final LiveData<List<Movie>> movies = favoriteMoviesDatabase.movieDao().loadAllMovies();

            movies.observe(this, new Observer<List<Movie>>() {
                @Override
                /* When movies are added or deleted from favorite_movies database,
                send the updatedMovies to the moviesDisplayAdapter */
